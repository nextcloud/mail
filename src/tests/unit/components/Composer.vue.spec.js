...
describe('Composer', () => {
	...
	it('leaves html without blob images untouched', async () => {
		...
		await expect(view.vm.embedBlobImages(html)).resolves.toEqual(html)
	})

	it('embeds blob images as base64 before the draft leaves the composer', async () => {
		const view = shallowMount(Composer, {
			propsData: {
				isFirstOpen: true,
				accounts: [
		...
		const result = await view.vm.embedBlobImages('<p><img src="blob:http://localhost/fake-id"></p>')

		expect(fetch).toHaveBeenCalledWith('blob:http://localhost/fake-id')
		expect(result).toMatch(/^<p><img src="data:image\/png;base64,/)
