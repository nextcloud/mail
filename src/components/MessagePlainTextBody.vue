...
export default {
	...
	computed: {
		enhancedBody() {
			return this.body.replace(/(^&gt;.*\n)+/gm, (match) => {
				return `<details class="quoted-text" open><summary>${t(
					"mail",
					"Quoted text"
	...
	async mounted() {
		if (this.enabledFreePrompt && this.message) {
			this.needsTranslation = await needsTranslation(
				this.message.databaseId
			);
		}
	},
	methods: {
...
<style lang="scss">
.quoted-text {
	color: var(--color-text-maxcontrast) summary {
		cursor: pointer;
	}
}
</style>
