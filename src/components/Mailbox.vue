...
export default {
	...
	methods: {
		...
		async loadEnvelopes() {
			...
			try {
				this.envelopes = await this.$store.dispatch('fetchEnvelopes', {
					accountId: this.account.id,
					folderId: this.folder.id,
					query: this.searchQuery,
				})

				logger.debug('envelopes fetched', {envelopes: this.envelopes})

				this.loadingEnvelopes = false

				if (!this.isMobile && this.$route.name !== 'message' && this.envelopes.length > 0) {
					// Show first message
					let first = this.envelopes[0]
		...
		async loadMore() {
			...
			try {
				const nextPage = await this.$store.dispatch('fetchNextEnvelopePage', {
				...
				this.envelopes = this.envelopes.concat(nextPage)
			} catch (error) {
