...
class QuickActionsMigrationServiceTest extends TestCase {
	...
	private const QUICK_STEP_MAILBOX_ID = 5;
	private const QUICK_STEP_TAG_ID = 2;

	/** Mappings of previously imported accounts and mailboxes, keyed by their id in the export. */
	private const ACCOUNT_MAPPING = [self::QUICK_STEP_ACCOUNT_ID => 100];
	private const MAILBOX_MAPPING = [self::QUICK_STEP_MAILBOX_ID => 200];
	private const TAG_MAPPING = [self::QUICK_STEP_TAG_ID => 22];
	...
	public function testEstimatedExportSizeCountsActionsAndSteps(): void {
		...
		$expectedSteps = array_sum(array_map(
			static fn (Actions $quickAction): int => count($quickAction->jsonSerialize()['actionSteps']),
			$quickActions,
		));
		self::assertSame(
			count($quickActions) * QuickActionsMigrationService::ESTIMATED_QUICK_ACTION_SIZE
				+ $expectedSteps * QuickActionsMigrationService::ESTIMATED_ACTION_STEP_SIZE,
			$size,
		);
	}
	...
	public function testImportMultipleQuickActions(): void {
		...
		$accountMapping = [self::QUICK_STEP_ACCOUNT_ID => rand(100, 199)];
		$mailboxMapping = [self::QUICK_STEP_MAILBOX_ID => rand(200, 299)];
		...
		$expectedSteps = [
			...
			['name' => 'moveThread', 'order' => 2, 'actionIndex' => 0, 'tagId' => null, 'mailboxId' => $mailboxMapping[self::QUICK_STEP_MAILBOX_ID]],
	...
	public function testImportNoQuickActions(): void {
		...
		$this->migrationService->importQuickActions($this->user, $this->importSource, $this->output,
			self::ACCOUNT_MAPPING, self::MAILBOX_MAPPING, []);
	}
	...
	public static function provideIncompleteMappings(): array {
		return [
			'no accounts imported' => ['accountMapping' => [], 'mailboxMapping' => self::MAILBOX_MAPPING],
			'nothing imported' => ['accountMapping' => [], 'mailboxMapping' => []],
		];
	}

	/**
	 * @dataProvider provideIncompleteMappings
	 */
	public function testImportWithoutAccountMappingIsSkipped(array $accountMapping,
		array $mailboxMapping): void {
	...
	public function testImportSkipsOnlyTheQuickActionWhoseMailboxIsMissing(): void {
		$this->importSource->method('getFileContents')
			->with(QuickActionsMigrationService::QUICK_ACTIONS_FILE)
			->willReturn(json_encode($this->getQuickActions()));

		$createdNames = [];
		$this->serviceMock->getParameter('quickActionsService')
			->method('create')
			->willReturnCallback(function (string $name, int $accountId) use (&$createdNames): Actions {
				$createdNames[] = $name;
				$created = new Actions();
				$created->setId(count($createdNames));
				return $created;
			});
		$this->serviceMock->getParameter('quickActionsService')
			->expects(self::exactly(2))
			->method('createActionStep')
			->willReturn(new ActionStep());

		// The first action moves a thread into an unmapped mailbox; the second
		// only needs the tag, so it has to survive on its own.
		$this->migrationService->importQuickActions($this->user, $this->importSource, $this->output,
			self::ACCOUNT_MAPPING, [], self::TAG_MAPPING);

		self::assertSame(['Second quick action'], $createdNames);
	}

	public function testImportSortsActionStepsTheArchiveListedOutOfOrder(): void {
		$quickAction = new Actions();
		$quickAction->setId(self::FIRST_QUICK_STEP_ID);
		$quickAction->setAccountId(self::QUICK_STEP_ACCOUNT_ID);
		$quickAction->setName('Out of order');

		$second = new ActionStep();
		$second->setId(2);
		$second->setActionId(self::QUICK_STEP_ACCOUNT_ID);
		$second->setName('markAsImportant');
		$second->setOrder(2);

		$first = new ActionStep();
		$first->setId(1);
		$first->setActionId(self::QUICK_STEP_ACCOUNT_ID);
		$first->setName('markAsRead');
		$first->setOrder(1);

		$quickAction->setActionSteps([$second, $first]);

		$this->importSource->method('getFileContents')
			->with(QuickActionsMigrationService::QUICK_ACTIONS_FILE)
			->willReturn(json_encode([$quickAction]));

		$created = new Actions();
		$created->setId(42);
		$this->serviceMock->getParameter('quickActionsService')->method('create')->willReturn($created);

		$orders = [];
		$this->serviceMock->getParameter('quickActionsService')
			->method('createActionStep')
			->willReturnCallback(function (string $name, int $order) use (&$orders): ActionStep {
				$orders[] = $order;
				return new ActionStep();
			});

		$this->migrationService->importQuickActions($this->user, $this->importSource, $this->output,
			self::ACCOUNT_MAPPING, self::MAILBOX_MAPPING, self::TAG_MAPPING);

		self::assertSame([1, 2], $orders);
	}

	public function testImportSkipsAQuickActionTheServiceRejects(): void {
		$this->importSource->method('getFileContents')
			->with(QuickActionsMigrationService::QUICK_ACTIONS_FILE)
			->willReturn(json_encode([$this->getFirstQuickAction()]));
		...
		$this->serviceMock->getParameter('quickActionsService')
			->method('createActionStep')
			->willThrowException(new ServiceException('Invalid action step order'));
		...
		// must not escape and abort the rest of the import
		$this->migrationService->importQuickActions($this->user, $this->importSource, $this->output,
			self::ACCOUNT_MAPPING, self::MAILBOX_MAPPING, self::TAG_MAPPING);
	}

	public function testImportSkipsQuickActionsWithAnUnknownStepType(): void {
		$quickAction = new Actions();
		$quickAction->setId(self::FIRST_QUICK_STEP_ID);
		$quickAction->setAccountId(self::QUICK_STEP_ACCOUNT_ID);
		$quickAction->setName('From a newer instance');

		$step = new ActionStep();
		$step->setId(1);
		$step->setActionId(self::QUICK_STEP_ACCOUNT_ID);
		$step->setName('teleportToMars');
		$step->setOrder(1);
		$quickAction->setActionSteps([$step]);
	...
	public function testImportWithoutMailboxesStillImportsMailboxlessQuickActions(): void {
		$quickAction = new Actions();
		$quickAction->setId(self::FIRST_QUICK_STEP_ID);
		$quickAction->setAccountId(self::QUICK_STEP_ACCOUNT_ID);
		$quickAction->setName('Mark as read');

		$step = new ActionStep();
		$step->setId(1);
		$step->setActionId(self::QUICK_STEP_ACCOUNT_ID);
		$step->setName('markAsRead');
		$step->setOrder(1);
		$quickAction->setActionSteps([$step]);
		...
		$this->serviceMock->getParameter('quickActionsService')
			->expects(self::once())
			->method('create')
			->with('Mark as read', self::ACCOUNT_MAPPING[self::QUICK_STEP_ACCOUNT_ID])
			->willReturn($created);
		...
		$this->serviceMock->getParameter('quickActionsService')
			...
			->willReturnCallback(function (string $name,
				...
				int $actionId,
				?int $tagId = null,
				?int $mailboxId = null) use (&$createdSteps): ActionStep {
				$createdSteps[] = ['name' => $name, 'order' => $order, 'actionId' => $actionId,
					'tagId' => $tagId, 'mailboxId' => $mailboxId];
				return new ActionStep();
			});
		...
		self::assertSame(
			[['name' => 'markAsRead', 'order' => 1, 'actionId' => 42, 'tagId' => null, 'mailboxId' => null]],
			$createdSteps,
		);
	}
	...
	private function getActionStepsForFirstQuickAction(): array {
		...
		$secondStep->setMailboxId(self::QUICK_STEP_MAILBOX_ID);
	...
	private function getActionStepsForSecondQuickAction(): array {
		...
		$secondStep->setTagId(self::QUICK_STEP_TAG_ID);
