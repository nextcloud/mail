...
class InternalAddressesMigrationServiceTest extends TestCase {
	...
	public function testEstimatedExportSizeScalesWithAddressCount(): void {
		$this->serviceMock->getParameter('internalAddressService')
			->method('getInternalAddresses')
			->with(self::USER_ID)
			->willReturn([$this->getTrustedIndividual(), $this->getTrustedDomain()]);

		$size = $this->migrationService->getEstimatedExportSize($this->user);

		self::assertSame(2 * InternalAddressesMigrationService::ESTIMATED_INTERNAL_ADDRESS_SIZE, $size);
	}
