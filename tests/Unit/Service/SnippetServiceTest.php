...
class SnippetServiceTest extends TestCase {
	...
	public function testUpdateSnippetDoesNotExist(): void {
		$snippetId = 1;
		$userId = 'bob';
		...
		$this->expectExceptionMessage('Snippet does not exist');

		$this->snippetService->update($snippetId, $userId, 'title', 'content');
	}
	...
	public function testDeleteSnippetDoesNotExist(): void {
		$snippetId = 1;
		$userId = 'bob';
	...
	public function testFindAllSharedWithMe(): void {
		...
		$shares = [new SnippetShare(), new SnippetShare()];
		...
		$result = $this->snippetService->findAllSharedWithMe($userId);
	...
	public function testFindAllSharedWithMeNoUser(): void {
		...
		$this->snippetMapper->expects($this->never())
			->method('findSharedWithMe');

		$this->expectException(DoesNotExistException::class);
		$this->expectExceptionMessage('User does not exist');
		$this->snippetService->findAllSharedWithMe($userId);
	...
	public function testFind(): void {
		...
		$result = $this->snippetService->find($snippetId, $userId);
	...
	public function testGetShares(): void {
		...
		$this->snippetShareMapper->expects($this->once())
			->method('findSnippetShares')
			->with($snippetId)
			->willReturn($shares);

		$result = $this->snippetService->getShares($snippetId);
