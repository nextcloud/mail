...
final class TextBlockServiceTest extends TestCase {
	...
	private \PHPUnit\Framework\MockObject\MockObject $textBlockMapper;

	/** @var TextBlockShareMapper|MockObject */
	private \PHPUnit\Framework\MockObject\MockObject $textBlockShareMapper;
	...
	private \OCA\Mail\Service\TextBlockService $textBlockService;
	...
	public function testCreate(): void {
		...
		$this->textBlockMapper->expects($this->once())
			->method('insert')
			->willReturnCallback(function (TextBlock $s) use ($userId, $title, $content, $textBlock): \OCA\Mail\Db\TextBlock {
	...
	public function testUpdate(): void {
		...
		$this->textBlockMapper->expects($this->once())
			->method('update')
			->willReturnCallback(function (TextBlock $s) use ($title, $content, $textBlock): \OCA\Mail\Db\TextBlock {
				$this->assertSame($title, $s->getTitle());
				$this->assertSame($content, $s->getContent());
				return $textBlock;
			});
	...
	public function testFindAllSharedWithMeNoUser(): void {
		$userId = 'alice';
		[new TextBlockShare(), new TextBlockShare()];
