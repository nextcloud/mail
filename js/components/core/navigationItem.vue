...
<template>
	...
	<nav-element v-else :id="item.id" v-bind="navElement(item)"
		...
		<!-- Bullet -->
		<div v-if="item.bullet" class="app-navigation-entry-bullet"
			 :style="{ backgroundColor: item.bullet }"></div>

		<!-- Main link -->
		<a :href="(item.href) ? item.href : '#' " @click="toggleCollapse"
		   :class="item.icon">
		...
		<div v-if="item.utils" class="app-navigation-entry-utils">
			<ul>
				...
				<li v-if="item.utils.actions && item.utils.actions.length === 1"
					...
					<button @click="item.utils.actions[0].action"
							:class="item.utils.actions[0].icon"
							:title="item.utils.actions[0].text"></button>
				</li>
				...
				<li v-else-if="item.utils.actions && item.utils.actions.length === 2 && !Number.isInteger(item.utils.counter)"
					...
					class="app-navigation-entry-utils-menu-button">
					<button @click="action.action" :class="action.icon"
							:title="action.text"></button>
				</li>
		...
		<div class="app-navigation-entry-deleted" v-if="item.undo">
			...
			<button class="app-navigation-entry-deleted-button icon-history"
					:title="t('settings', 'Undo')"></button>
		</div>
		...
		<ul v-if="item.children">
			<navigation-item v-for="(item, key) in item.children" :item="item"
							 :key="key"/>
		</ul>
	</nav-element>
</template>
