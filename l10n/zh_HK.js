OC.L10N.register(
    ...
    "Mail server configuration mode" : "郵件伺服器設定模式",
    "Auto" : "自動",
    "Manual" : "手動",
    "Name" : "姓名",
    "Mail address" : "電郵地址",
    ...
    "Actions" : "操作",
    "Add action" : "添加操作",
    "Redirected messages might be rejected by the receiving server, and cannot be read by the recipient if encrypted." : "重新導向的郵件可能會被接收伺服器拒絕；如郵件已加密，收件者亦可能無法閱讀。",
    "Priority" : "優先",
    "Enable filter" : "啟用篩選器",
    "Tag" : "標籤",
    "delete" : "刪除",
    ...
    "Using master user and master password" : "在使用主用戶及主密碼",
    "Using master password for all users" : "所有用戶均使用主密碼",
    ...
    "Without a master user, all users authenticate with their normal username and this master password." : "如沒有設定主用戶，所有用戶均會使用其一般用戶名稱及此主密碼進行驗證。",
    "Master user" : "主用戶",
    "e.g. {example}" : "如 {example}",
    "Appended to the username. Must start with the same separator as {setting} in Dovecot." : "附加至用戶名稱之後。開頭必須與 Dovecot 中的{setting} 使用相同的分隔符號。",
    "Sieve" : "Sieve",
    ...
    "* %USERID% and %EMAIL% will be replaced with the user's UID and email. {ldapPlaceholder} will be replaced with the value of that LDAP attribute" : "* %USERID% 及 %EMAIL% 會分別替換為用戶的 UID 及電郵地址。{ldapPlaceholder} 會替換為該 LDAP 屬性的值",
