OC.L10N.register(
    ...
    "Actions" : "動作",
    "Add action" : "新增動作",
    "Redirected messages might be rejected by the receiving server, and cannot be read by the recipient if encrypted." : "轉寄的訊息可能會被收件伺服器拒絕，且若經過加密，收件人將無法閱讀。",
    "Priority" : "優先",
    "Enable filter" : "啟用篩選器",
    "Tag" : "標籤",
    "delete" : "刪除",
    ...
    "Using master user and master password" : "使用主控使用者與主控密碼",
    "Using master password for all users" : "為所有使用者使用主控密碼",
    ...
    "Without a master user, all users authenticate with their normal username and this master password." : "若無主控使用者，所有使用者皆須使用其一般使用者名稱及此主控密碼進行驗證。",
    "Master user" : "主控使用者",
    "e.g. {example}" : "例如 {example}",
    "Appended to the username. Must start with the same separator as {setting} in Dovecot." : "附加於使用者名稱之後。必須以與 Dovecot 中 {setting} 相同的分隔符開頭。",
    "Sieve" : "過濾",
    ...
    "* %USERID% and %EMAIL% will be replaced with the user's UID and email. {ldapPlaceholder} will be replaced with the value of that LDAP attribute" : "* %USERID% 與 %EMAIL% 將會被使用者的 UID 與電子郵件取代。{ldapPlaceholder} 將會以 LDAP 屬性的值取代",
