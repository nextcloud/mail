OC.L10N.register(
    ...
    "Flag" : "Флаг",
    "The stop action ends all processing" : "Остановка прекратит всё что выполняется",
    "Operator" : "Оператор",
    "Update mail filter" : "Обновить почтовый фильтр",
    "Filter name" : "Имя фильтра",
    "If all the conditions are met" : "Если все условия удовлетворены",
    "If any of the conditions are met" : "Если любое из условий удовлетворено",
    "Tests are applied to incoming emails on your mail server, targeting fields such as subject (the email's subject line), from (the sender), and to (the recipient). You can use the following operators to define conditions for these fields:" : "Тестирование входящих писем на вашем почтовом сервере выполняется по таким полям, как «тема» (тема письма), «от» (отправитель) и «кому» (получатель). Вы можете использовать следующие операторы для определения условий для этих полей:",
    "Then perform these actions" : "Затем выполнить следующие действия",
