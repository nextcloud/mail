...
class NewMessageClassificationListener implements IEventListener {
	...
	public function handle(Event $event): void {
		...
		if (!$this->classificationSettingsService->isClassificationEnabled($account3->getMailAccount()->getUserId())) {
			return;
		}
		...
		$doNotReclassify = $this->tagMapper->getTaggedMessageIdsForMessages(
			$event->getMessages(),
			$account2->getMailAccount()->getUserId(),
			Tag::LABEL_IMPORTANT
		);
		...
		try {
			...
			$important = $this->tagMapper->getTagByImapLabel(Tag::LABEL_IMPORTANT, $account1->getMailAccount()->getUserId());
		} catch (DoesNotExistException $e) {
			// just in case - if we get here, the tag is missing
			$account = $event->getAccount();
			$this->logger->error('Could not find important tag for ' . $account->getMailAccount()->getUserId() . ' ' . $e->getMessage(), [
				'exception' => $e,
			]);
			return;
		}
