...
class TextBlocksMigrationService {
	...
	public const ESTIMATED_TEXT_BLOCK_SIZE = 100;
	...
	/**
	 * The content is the only part that grows meaningfully, so it is measured
	 * instead of estimated.
	 */
	public function getEstimatedExportSize(IUser $user): int {
		$bytes = 0;

		foreach ($this->textBlockService->findAll($user->getUID()) as $textBlock) {
			$bytes += self::ESTIMATED_TEXT_BLOCK_SIZE
				+ strlen($textBlock->getTitle())
				+ strlen($textBlock->getContent())
				+ strlen((string)$textBlock->getPreview());
		}
