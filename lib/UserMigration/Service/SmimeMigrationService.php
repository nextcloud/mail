...
class SmimeMigrationService {
	...
	public const ESTIMATED_CERTIFICATE_SIZE = 100;
	...
	/**
	 * Measured on the stored ciphertext, which is longer than the plaintext
	 * written to the archive, so this overestimates rather than decrypting.
	 */
	public function getEstimatedExportSize(IUser $user): int {
		$bytes = 0;

		foreach ($this->smimeService->findAllCertificates($user->getUID()) as $certificate) {
			$bytes += self::ESTIMATED_CERTIFICATE_SIZE
				+ strlen($certificate->getCertificate())
				+ strlen((string)$certificate->getPrivateKey());
		}
	...
	public function importCertificates(IUser $user,
		...
		// The export writes no directory entries, so the folder listing is the only
		// way to find out whether anything was exported.
		$certificateFileNames = $importSource->getFolderListing(self::SMIME_CERTIFICATE_FOLDER);

		foreach ($certificateFileNames as $certificateFileName) {
			if (!str_ends_with($certificateFileName, '.json')) {
				continue;
			}

			// getFolderListing() returns names relative to the folder
			$certificateFilePath = self::SMIME_CERTIFICATE_FOLDER . $certificateFileName;
