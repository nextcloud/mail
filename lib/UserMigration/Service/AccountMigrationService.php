...
class AccountMigrationService {
	...
	public function deleteAllAccounts(IUser $user, OutputInterface $output): void {
		$output->writeln(
			$this->l10n->t('Deleting mail accounts for user %s', [$user->getUID()]),
		...
		foreach ($allAccounts as $account) {
			if ($account->getMailAccount()->getProvisioningId() !== null) {
				$output->writeln("Skipping deletion of provisioned account {$account->getId()}");
				continue;
			}
	...
	public function exportAccounts(IUser $user, IExportDestination $exportDestination, OutputInterface $output): void {
		...
		foreach ($accounts as $account) {
			...
			try {
				$this->getDecryptedPasswords($mailAccount, $accountData);
				$this->getDecryptedOauthToken($mailAccount, $accountData);
				$this->getDecryptedSievePassword($mailAccount, $accountData);
	 ...
	 */
	public function importAccounts(IUser $user, IImportSource $importSource, OutputInterface $output, array $certificatesMapping): array {
		$output->writeln(
	 ...
	 * @throws \Exception
	 */
	private function getDecryptedSievePassword(MailAccount $mailAccount, array &$accountData): void {
