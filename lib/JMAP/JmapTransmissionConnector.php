...
class JmapTransmissionConnector implements ITransmissionConnector {
	...
	#[\Override]
	public function sendMessage(Account $account, LocalMessage $lMessage, Mailbox $sentMailbox): void {
		$to = $this->transmissionService->getAddressList($lMessage, Recipient::TYPE_TO);
		...
		try {
			...
			$lMessage->setStatus(LocalMessage::STATUS_PROCESSED);
		} catch (Exception $e) {
			$status = $this->classifyJmapError($e->getMessage());
			$this->logger->error('JMAP send failed: ' . $e->getMessage(), ['exception' => $e]);
			$lMessage->setStatus($status);
		}
	}

	#[\Override]
	public function saveMessage(Account $account, Mailbox $mailbox, LocalMessage $lMessage, array $flags = []): void {
		...
		$jMessage->in($remoteId);
		if ($keywords !== []) {
			$jMessage->keywords($keywords);
		}

		$this->jmapOperationsService->connect($account);
		try {
			$this->jmapOperationsService->entitySave($jMessage, $attachments);
		} catch (Exception $e) {
	 ...
	 */
	private function convertLocalMessage(Address $from, AddressList $to, AddressList $cc, AddressList $bcc, LocalMessage $lMessage): MailParametersRequest {
		$jMessage = new MailParametersRequest();
		$jMessage->from($from->getEmail() ?? '', $from->getLabel() ?? '');

		foreach ($to->iterate() as $address) {
			$jMessage->to($address->getEmail() ?? '', $address->getLabel() ?? '');
		}

		foreach ($cc->iterate() as $address) {
			$jMessage->cc($address->getEmail() ?? '', $address->getLabel() ?? '');
		}

		foreach ($bcc->iterate() as $address) {
			$jMessage->bcc($address->getEmail() ?? '', $address->getLabel() ?? '');
		}

		if (($inReplyTo = $lMessage->getInReplyToMessageId()) !== null) {
			$jMessage->inReplyTo($inReplyTo);
			$jMessage->references($inReplyTo);
		}

		$jMessage->subject($lMessage->getSubject() ?? '');

		$bodyPlain = $lMessage->getBodyPlain();
		$bodyHtml = $lMessage->getBodyHtml();

		$bodyPart = $jMessage->bodyPartStructure();
		$bodyPart->type('multipart/mixed');

		if (!empty($bodyPlain) && !empty($bodyHtml)) {
			$textPart = $bodyPart->addPart();
			$textPart->type('multipart/alternative');
			...
			$textPart->addPart()
				->id('text-html')
				->type('text/html');
			$jMessage->bodyPartValue('text-plain', $bodyPlain);
			$jMessage->bodyPartValue('text-html', $bodyHtml);
	...
	/**
	 * Resolve a message's local attachments into their raw content, skipping any
	 * that no longer exist. JmapOperationsService uploads this content and wires
	 * the resulting blob into the email.
	 *
	 * @return array<array{content: string, type: string, name: string, disposition: ?string, contentId: ?string}>
	 */
	private function collectAttachments(Account $account, LocalMessage $message): array {
		$attachments = [];
		foreach ($this->transmissionService->getAttachments($message) as $attachmentRef) {
			$content = $this->transmissionService->getAttachmentContent($account, $attachmentRef);
			if ($content === null) {
				continue;
			}
			$attachments[] = $content;
